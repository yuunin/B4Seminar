...
for i = 1:size(train, 3)
    indices = randperm(img_size(1)*img_size(2), numElem);
    img_1 = train(:,:,i);
    img_2 = reshape(img_1', 1,img_size(1)*img_size(2));
    img_3 = zeros(1,img_size(1)*img_size(2));
    img_3(indices) = img_2(indices);
    img_4 = uint8(reshape(img_3, [img_size(1), img_size(2)]))';
...
%train network
[net, trainInfo] = trainNetwork(Train, TrainLabel, layers, options);
...
ValidationAccuracy = Accuracy / 418*100
%%
